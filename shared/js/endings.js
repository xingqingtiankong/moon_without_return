...
const entries=Object.fromEntries(rows.map(([id,title,medal,intro,text,path],i)=>[id,{id,title,medal,intro,text,path,index:i,overlay:['late_letter','piano_intact'].includes(id)}]));const key='moon_without_return_ending_medals_v1';
function load(){try{const s=JSON.parse(localStorage.getItem(key)||'{}');return s&&typeof s==='object'&&!Array.isArray(s)?s:{};}catch{return {};}}
function unlock(ids){const s=load(),fresh=[];for(const id of ids)if(entries[id]&&!s[id]){s[id]={at:new Date().toISOString()};fresh.push(id);}try{localStorage.setItem(key,JSON.stringify(s));}catch{return [];}if(fresh.length)window.dispatchEvent(new CustomEvent('moon:achievement-unlocked',{detail:fresh}));return fresh;}


const medalBase=(()=>{try{const s=document.currentScript&&document.currentScript.src;if(s)return new URL('../../img/ui/ending-medals/',s).href;}catch(e){}return '../img/ui/ending-medals/';})();
function medal(id,locked=false){const e=entries[id];if(!e)return '';return '<img class="medal-badge'+(locked?' is-locked':'')+'" src="'+medalBase+e.id+'.svg" alt="'+e.medal+'" title="'+e.title+' · '+e.medal+'" width="80" height="92" draggable="false">';}
function auth(s,draft=false){const c=s.choices||{},f=s.flags||{},v=(k,...x)=>x.includes(c[k]),a=c.FINAL;if(!draft&&a!=='F-A')return null;if(v('C14','C14C')&&v('C15','C15A'))return 'U2';if(v('C01','C01B')&&v('C02','C02B')&&v('C05','C05A')&&f.witness_complete&&v('C06','C06A')&&(v('C10','C10A')||f.archive_status==='independent')&&v('C15','C15B'))return 'U3';if(v('C05','C05A','C05B')&&(v('C10','C10A','C10C')||['independent','readonly'].includes(f.archive_status))&&v('C15','C15A'))return 'U1';if(v('C01','C01C')&&v('C09','C09A')&&v('C15','C15C'))return 'U4';return null;}
function resolve(s){const c=s.choices||{},f=s.flags||{},v=(k,...x)=>x.includes(c[k]),u=auth(s),final=c.FINAL,rescue=v('C12','C12A')||v('C14','C14A'),ind=v('C13','C13B','C13C'),informed=v('C06','C06A','C06B')&&c.K_AZHI!=='K-C',closed=f.azhi_statement==='close';let id;
