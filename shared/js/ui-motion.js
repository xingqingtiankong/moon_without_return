...
window.MoonUiMotion=(()=>{
    ...
    function transition(element,visible,kind="window"){
        ...
        const state={animations:[]};running.set(element,state);
        element.hidden=false;element.inert=!visible;
        ...
        if(reduced()||!element.animate){finish();return Promise.resolve(true);}
        ...
        content.forEach((child,index)=>state.animations.push(child.animate([
            {translate:offsets[index]},
            {translate:visible?"0 0px":kind==="hud"?"0 -6px":"0 8px"}
        ],options)));
    ...
    function statusHuds(elements){
        let obscured=false;
        const entries=[...elements].map(element=>({element,value:element.textContent.trim(),version:0,timer:0,pending:false}));
        function pulse(entry){
            clearTimeout(entry.timer);
            const version=++entry.version;
            if(obscured){entry.pending=true;return;}
            entry.pending=false;
        ...
        return {
            obscure(value){
                ...
                entries.forEach(entry=>{
                    if(value){clearTimeout(entry.timer);entry.version++;transition(entry.element,false,"hud");}
                    else if(entry.pending)pulse(entry);
                });
            },
            dispose(){observers.forEach(observer=>observer.disconnect());entries.forEach(entry=>{clearTimeout(entry.timer);entry.version++;});}
        };
    }
    return Object.freeze({show:(element,kind)=>transition(element,true,kind),hide:(element,kind)=>transition(element,false,kind),statusHuds});
})();
