"use strict";
(function(){
const n=MoonCampaign.nodes,p=MoonPlacements,pt=(map,x=900,y=560,radius=110)=>({map,x,y,radius}),L=(actor,text)=>({type:'line',actor,text}),W=t=>L('wukang',t),G=t=>L('guanghan',t),A=t=>L('azhi',t),X=t=>L('xing8',t),S=t=>L('system',t),T=t=>W('（'+t+'）'),AR=t=>L('archivewukang',t),wake={map:'R03',x:860,y:480},home={map:'F01',x:755.4,y:743.7},desk=MoonCampaign.points.desk,core=pt('B04'),archive=pt('B03',780,590),comm=pt('R06',880,460),rescue=pt('R08'),table=p.h01_dinner;
function add(id,title,point,queue,next,extra={}){n[id]={id,title,targets:point?[{...point,id:id+'_action',label:title,kind:'campaign'}]:[],queue,next,...(point===p.h01_azhi?{actor:'azhi'}:{}),...extra};}
function auto(id,title,queue,next,extra={}){add(id,title,null,queue,next,{auto:true,...extra});}
function game(id,title,point,key,queue,next,extra={}){add(id,title,point,[],next,{game:key,success:queue,failure:[G('辅助路径接管剩余步骤，原始资料不会丢失。'),...queue],...extra});}
function choose(id,title,point,queue,key,options,extra={}){add(id,title,point,queue,null,{choice:{id:key,text:title,options},...extra});}
const opt=(id,text,node,flags={})=>({id,text,effects:{node,flags}}),ev=(...evidence)=>({evidence});
function entry(id,title,source,text){MoonEvidence.entries[id]={id,title,source,text};}

for(const [id,key,title]of [['P01_TOOLS','memory','比对领用工具与登记样本'],['H02_D5_DRONE','huarong','移开舱柜，让无人机主舱归位'],['H02_D6_COMMS','number-wordle','校验通信数字码'],['H04_COLD','link','连接同源缓存片段']])if(n[id]){n[id].game=key;n[id].title=title;}
Object.assign(MoonLater.chapters,{4:{title:'第四章 · 谁是凶手',next:'T_START',summary:'终止命令到过这里，却没有通过认证。下一步是取得当前个体能够使用的新授权。'},5:{title:'第五章 · 证明它已经结束',next:'E_START',summary:'地球签发了新的终止授权。接下来由当前操作员提交，并为所有离站者保留合法身份。'},6:{title:'第六章 · 最后一个夜晚',next:'F_START',summary:'终止草案待执行。阿芷独立转移，小星单独封存。家庭入口已关闭。'},7:{title:'终章 · 第一次离开',summary:'培养系统没有启动第十八个人。第十七个人离开了广寒宫。至于哪里算家，没有系统再替他回答。'}});MoonLater.chapters[3].next='K_START';
auto('K_START','第四章 · 谁是凶手',[S('第236天。回传灯仍是黄色，没有送达凭据。家庭恢复程序传来阿芷的请求。'),G('阿芷希望亲自讨论儿童资料的处理。'),W('我进去听她说。')],'K_AZHI',{warpNext:home,effects:{chapter:4,day:236,chapterComplete:false}});
...
game('K_INDEX','连接最高日志的同源索引',pt('B01',730,620),'link',[G('日志层恢复，只读权限已开放。'),W('从第一条开始，一条一条看。')],'K_LOG1',{failureGroup:'core',finalGroupPenalty:20,effects:{flags:{core_access:true}}});
...
logs.forEach(([title,text,thought],i)=>add('K_LOG'+(i+1),'插入记录 '+(i+1)+'：'+title,pt('B04',...ring[i],120),[S(text),T(thought)],i===6?'K_QUESTION':'K_LOG'+(i+2),{effects:{evidence:['K_RULE_'+(i+1)],flags:{core_rule_count:i+1}}}));
add('K_QUESTION','核对仍在运行的规则',core,[S('腕端重新同步：剩余65天。任务仍被计为正常的一年。'),W('人早就死了，命令也到了。为什么现在还显示六十五天？'),G('当前规则集中不存在有效终止授权。'),W('家庭呢？让人以为还能回去，是谁决定的？'),G('“归家预期”稳定协议由人类心理团队批准。最高日志含批准页与调用限制。'),W('它没有把人送回去，只让人一次次撑到下一班。'),G('批准页与执行记录可以导出。'),T('执行者是它。可门上的规则不是它写的。'),W('现在的目标不是证明我是谁的替身。我要证明这个计划已经结束。')],'K_LAST_REBUILD',{effects:{day:300,evidence:['E06','K_HUMAN_PROTOCOL']}});
add('K_LAST_REBUILD','载入最后一次权限重建',pt('R03',1010,355),[W('带上原始权限结构，不要补人物记忆。'),G('将载入档案武康。')],'K_SIGN_FADE',{warpNext:{map:'A02',x:780,y:570}});
...
add('K_WITNESS_EXPORT','导出见证声明与权限图',desk,[S('导出：旧权限结构、损坏字段位置、独立见证声明。'),AR('我能证明这套身份怎样工作。我不能证明我们是同一个人。')],'CHAPTER4_COMPLETE',{warp:wake,effects:{chapterComplete:true,evidence:['K_AUTH_STRUCTURE','K_DAMAGED_FIELDS','K_WITNESS'],flags:{archive_witness:true,archive_missing:0}}});

auto('T_START','第五章 · 证明它已经结束',[S('第301天。16号的损坏留言、认证失败记录和旧权限图终于能放在一起。'),W('去档案室。这次我们知道缺失字段属于哪一层了。')],'T_MESSAGE',{effects:{chapter:5,day:301,chapterComplete:false},warpNext:wake});
...
add('T_VITAL','在休眠区采集连续生命体征',pt('R03',1010,355),[S('第330天。连续生命体征采样完成，来源为当前身体，不使用模板中的健康记录。'),G('检测到一名持续存活的生物个体。')],'T_COORD_ROUTE',{effects:{day:330,evidence:['T_VITALS']}});
...
add('T_ID_REQUEST','读取地球的身份确认请求',rescue,[S('第340天。地球回信：“请确认幸存者身份。”'),W('不是问原武康的档案在哪里。是在问活着的这个人。'),G('身份终端可登记 CURRENT OPERATOR。')],'T_NAME',{effects:{day:340,evidence:['T_ID_REQUEST']}});
add('T_NAME','登记当前操作员的新姓名',rescue,[W('就用这个名字。旧名字放在记忆来源里。'),AR('我为权限结构与字段映射作证。'),G('我为当前身体从唤醒至今的生命连续性作证。')],'T_CONFLICT',{form:'name',effects:{evidence:['T_NEW_ID','T_CONTINUITY']}});
...
add('T_PROJECT','核对项目编号并保存正式授权',comm,[S('项目编号：广寒宫。终止范围：继任激活、任务维持及附属恢复循环。'),G('三项校验均通过，U2正式终止授权已保存。'),W('现在轮到我提交。')],'CHAPTER5_COMPLETE',{effects:{chapterComplete:true,evidence:['U2'],flags:{termination_authorized:true}}});

auto('E_START','第六章 · 最后一个夜晚',[S('第351天。新的授权不替任何人决定关系，也不自动执行停机。'),G('请在核心终止台拼合四项材料。')],'E_MAP',{effects:{chapter:6,day:351,chapterComplete:false}});
...
add('E_SUBMIT','核对申请人并长按提交',core,[G('终止草案已进入待执行。生效日为第365天，今天不会提前关闭生命支持。'),W('留出时间，完成撤离和人格转移。')],'E_HOME',{form:'hold',holdSeconds:2,formText:'确认四类材料齐全。申请人必须是当前操作员。',effects:ev('E_TERMINATION_DRAFT')});
add('E_HOME','在休眠区进入家庭空间',pt('R03',1010,355),[S('第364天夜。家庭空间当前不稳定。'),W('我进去看看阿芷。')],'E_TALK',{effects:{day:364,family_state:'S3',flags:{dishes_silent:true}},warpNext:home});
const questions=[['leave','你还想离开这里吗？','想。但不是作为你的妻子。'],['name','你想叫什么？','等启动以后再决定。'],['child','小星怎么办？','保留他，不替他长大。']];
add('E_TALK','听阿芷说自己的决定',p.h01_azhi,[A('你把外面的授权办下来了？'),W('办下来了。现在想听你怎么说。')],null,{prepare:s=>({queue:s.flags.night_started?[]:[A('你把外面的授权办下来了？'),W('办下来了。现在想听你怎么说。')],choice:{id:'night_questions',text:'还想问她什么？',options:[...questions.filter(([id])=>!s.flags['night_'+id]).map(([id,text])=>opt(id,text,'E_ANSWER_'+id)),...(questions.every(([id])=>s.flags['night_'+id])?[opt('continue','按你的意愿办理转移。','E_DECLARATION')]:[])]},effects:{flags:{night_started:true}}})});
questions.forEach(([id,text,answer])=>add('E_ANSWER_'+id,text,p.h01_azhi,[W(text),A(answer)],'E_TALK',{effects:{flags:{['night_'+id]:true}},repeatable:true}));
...
add('E_CHILD','单独封存 XING-8',table,[S('XING-8：单独封存。成年资料没有合并，也没有替其推进年龄。'),A('当前操作员……'),{type:'wait',seconds:1},A('这个也不像名字。')],'E_NAME_SHARE',{effects:{flags:{xing8_sealed:true},evidence:['E_TRANSFER_RECORD']}});
...
add('E_LEAVE','从入户门离开家庭空间',MoonDay3.exit,[S('家庭入口关闭。状态：人格核心待装载。'),T('这扇门不再把我送回同一顿晚饭。')],'CHAPTER6_COMPLETE',{warpNext:{map:'R01',x:800,y:600},effects:{chapterComplete:true,family_state:'closed',flags:{family_closed:true},evidence:['E_HOME_CLOSED']}});

auto('F_START','终章 · 第一次离开',[S('第365天。走廊没有新的报修，也没有催促音。'),G('终止台等待当前操作员。')],'F_AUTH',{effects:{chapter:7,day:365,chapterComplete:false}});
...
add('F_EXECUTE','保持三秒，执行终止',core,[G('广寒宫计划：终止。'),S('腕端的任务倒计时消失。'),W('去培养区。我要亲眼看见下一次没有开始。')],'F_SERVICE',{form:'hold',holdSeconds:3,formText:'执行后终止继任激活循环，保留撤离必需的生命支持。',effects:{flags:{plan_terminated:true},evidence:['F_TERMINATED']}});
...
add('F_WAIT','原地观察培养舱',pt('B02',820,560),[{type:'wait',seconds:3},S('没有开启的舱盖，也没有新的名字。'),W('这一项，完成。')],'F_GET_CORE',{effects:{flags:{cycle_stopped:true},evidence:['F_CYCLE_STOPPED']},unskippableWait:true});
...
add('F_CORE_LOADED','确认独立乘员装载完成',rescue,[S('数据舱：一名独立数字乘员。家庭恢复入口已永久移除。'),A('收到了。我等外部启动。')],'F_OXYGEN',{effects:{flags:{azhi_loaded:true},evidence:['F_INDEPENDENT_PASSENGER']}});
game('F_OXYGEN','最后一次氧气管路维护',rescue,'oxygen',[G('主通路已闭合。气密与供氧恢复，允许进入装备间。'),W('这次修好之后，就不用再回来值班了。')],'F_SUIT',{penalty:20,finalOxygen:true});
add('F_SUIT','穿戴航天服并确认氧量',rescue,[S('航天服氧量：100%。救援信标已锁定，没有新的维修计时。'),G('沿月面标记步行。保持呼吸。')],'F_MOON_MID',{effects:{flags:{outside_ready:true,oxygen:100}}});
add('F_MOON_MID','走到月面信标中段',pt('R09',950,600),[G('武康——'),{type:'wait',seconds:1},G('当前乘员身份记录已更新。请继续向信标行进。'),T('它停了一下。没有把那个旧名字叫完。')],'F_RESCUE',{effects:{flags:{oxygen:94}}});
add('F_RESCUE','在信标旁确认撤离人数',pt('R09',1200,650),[L('rescue','已接通。确认一名生物乘员、一份独立数字乘员数据。'),W('确认。另有单独封存的儿童模型，附在声明目录中。'),G('我的任务结束了。')],'F_EARTH',{effects:{flags:{rescued:true},evidence:['F_RESCUE_CONFIRMATION']}});
auto('F_EARTH','地球 · 隔离观察室',[S('隔离室的灯没有维修编号。玻璃另一边暂时没有人。'),T('我可以走过去，也可以先站一会儿。')],'F_GLASS',{warpNext:{map:'R01',x:800,y:600},effects:{flags:{epilogue:true}}});
...
auto('F_BLACK','还没有说完的回答',[S('小星吸气，像要说什么。画面在回答前暗下去。'),{type:'wait',seconds:2}],'F_AZHI_BOOT',{effects:{flags:{epilogue_black:true}}});
auto('F_AZHI_BOOT','另一处启动窗口',[],'GAME_COMPLETE',{prepare:s=>({queue:[S('阿芷的独立启动窗口亮起。默认姓名：AZHI。'),...(s.flags.name_shared?[S('她先看了一眼乘员记录上的“'+s.flags.player_name+'”，随后回到自己的姓名栏。')]:[]),S('她删除了默认姓名，光标停在空白处。'),{type:'wait',seconds:2},S('培养系统没有启动第十八个人。'),S('第十七个人离开了广寒宫。'),S('至于哪里算家，没有系统再替他回答。')]}),effects:{chapterComplete:true,flags:{game_complete:true,ending:'first_departure'},evidence:['F_END_RECORD']}});
auto('DREAM_END','第十八次值班：无梦休眠',[S('气密恢复，但当前个体无法完成撤离确认。广寒子转入生命维持与无梦休眠。'),G('当前个体生命支持保持。未启动新的继任体。'),S('终止已执行。等待撤离的，是仍然活着的第十七个人。')],'GAME_COMPLETE',{effects:{chapter:7,chapterComplete:true,flags:{game_complete:true,ending:'dream_sleep'}}});
...
proto.act=function(id,approach=false){const d=this.node;if(d?.form&&!this.story.busy&&d.targets.some(t=>t.id===id)){window.dispatchEvent(new CustomEvent('moon:final-form',{detail:{node:d.id}}));return true;}return oldAct.call(this,id,approach);};
proto.commit=function(d,result=null){if(d.id==='E_TALK'){const custom=d.prepare(this.story.state);return this.story.run({id:'night_question_menu',once:false,queue:[...custom.queue,{type:'choice',...custom.choice}],effects:custom.effects});}if(d.finalOxygen&&result&&!result.success&&this.story.state.mental_value<=20)d={...d,next:'DREAM_END'};return oldCommit.call(this,d,result);};
const oldDoor=MoonLater.door;globalThis.MoonLater=Object.freeze({...MoonLater,door(s,from,to){if(s.flags.epilogue)return '隔离观察尚未结束。可以走到玻璃前。';if(s.flags.family_closed&&to.startsWith('F'))return '家庭入口已关闭，人格核心待装载。';if(to==='B04')return s.flags.core_access?'':'最高日志尚需完成三层凭证校验。';if(to==='B05')return s.flags.bio_area_access?'':'生物循环区尚未授权检修。';return oldDoor(s,from,to);}});
...
function validName(raw){const value=raw.trim().normalize('NFC');if(!value||[...value].length>12||/[\p{C}\p{Zl}\p{Zp}]/u.test(value)||!/[\p{L}\p{N}]/u.test(value))return '请输入1—12个可显示字符，至少包含一个文字或数字。';if(reserved.has(value.toUpperCase().replace(/\s+/g,' ')))return '该名称保留给已有身份，请为当前个体选择新名字。';return '';}
