...
globalThis.MoonOverview=(()=>{
    const rooms=Object.freeze({
        F00:[.455,.80],F01:[.455,.635],F02:[.235,.305],F03:[.48,.31],F04:[.698,.29],F05:[.862,.515],
        R01:[.425,.865],R02:[.405,.46],R03:[.395,.205],R04:[.135,.505],R05:[.145,.165],R06:[.69,.19],R07:[.69,.435],R08:[.695,.69],R09:[.925,.69],
        B01:[.49,.34],B02:[.21,.265],B03:[.745,.28],B04:[.495,.72],B05:[.20,.715]
    });
    ...
    function render(mapId,state){
        ...
        root.replaceChildren();summary.replaceChildren();
        ...
        function marker(type,map,point,text,label,offset=0){
            ...
            element.style.left=`${point[0]*100}%`;element.style.top=`${point[1]*100}%`;
            element.style.setProperty("--marker-offset",`${offset}px`);
            element.dataset.map=map;element.setAttribute("role","img");element.setAttribute("aria-label",label);element.title=label;
            const dot=document.createElement("b");dot.textContent=type==="player"?"●":"!";
            const caption=document.createElement("span");caption.textContent=text;element.append(dot,caption);root.append(element);
        }
        if(current)marker("player",mapId,current,"你在这里",`当前位置：${mapId} · ${MoonMapConfig.MAPS[mapId].name}`);
        for(const target of targets){if(!groups.has(target.map))groups.set(target.map,[]);groups.get(target.map).push(target);}
        for(const [room,items] of groups){
            ...
            const item=document.createElement("li");item.dataset.map=room;
            item.textContent=`${room[0]===zone?"◆":"↗"} ${room} ${name}：${labels}${room[0]===zone?"":`（位于${zoneNames[room[0]]||"其他区域"}）`}`;
            summary.append(item);
        }
        if(!targets.length){const item=document.createElement("li");item.textContent="当前没有待完成的任务目标。";summary.append(item);}
    }
