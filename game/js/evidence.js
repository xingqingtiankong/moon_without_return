"use strict";
(function(){
 const entries={};function add(id,title,source,text,when){entries[id]={id,title,source,text,when};}
 ...
 function collected(s){const ids=new Set(s.evidence||[]);Object.values(entries).forEach(e=>{if(e.when?.(s))ids.add(e.id);});return [...ids].map(id=>entries[id]||{id,title:id,source:'旧存档',text:'该记录已保存。'});}
 function render(root,s){root.replaceChildren();const h=document.createElement('h2'),count=document.createElement('b'),intro=document.createElement('p'),list=document.createElement('div'),rows=collected(s);h.textContent='证据档案';count.textContent=rows.length+' 项';intro.textContent='按调查线索整理。展开查看来源、已确认的事实与尚未解决的问题。';list.className='evidence-cards';const ul=document.createElement("ul");ul.hidden=true;root.append(h,count,intro,ul,list);rows.forEach(e=>{const card=document.createElement('details'),summary=document.createElement('summary'),source=document.createElement('small'),body=document.createElement('p');card.dataset.evidence=e.id;summary.textContent=e.title;source.textContent=e.id+' · '+e.source;body.textContent=typeof e.text==='function'?e.text(s):e.text;card.append(summary,source,body);list.append(card);});if(!rows.length)intro.textContent='尚未取得重要记录。核验后的信息会自动保存在这里。';}
