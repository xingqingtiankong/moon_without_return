...
(function(){
    ...
    function enrichSaveState(state){
        const context=globalThis.MoonSaveContext;
        if(!context||typeof context.capture!=='function')return state;
        try{
            const meta=context.capture(state);
            return meta&&typeof meta==='object'?{...state,...meta}:state;
        }catch(error){
            console.warn('Unable to enrich save metadata.',error);
            return state;
        }
    }
    function runtimeState(value){
        return {...value,snapshot:null,snapshotScene:null,storyTitle:'',storySummary:'',chapterTitle:''};
    }
    class Story {
        #state; #pending=null; #tasks;
        constructor({state=storage.defaultGameState(),tasks=[]}={}){
            this.#tasks=new Map(tasks.map(t=>[t.id,copy(t)]));
            this.#state=runtimeState(storage.migrateSave(state)||storage.defaultGameState());
            ...
            if(this.positionReset){this.#state.currentMap='F01';this.#state.playerPosition={x:755.4,y:743.7};}
            this.#state.configVersion=config.CONFIG_VERSION;
        }
        get state(){return copy(this.#state);}
        get busy(){return this.#pending!==null;}
        get current(){return this.#pending?copy(this.#pending.queue[0]||null):null;}
        ...
        tasks(){return [...this.#tasks.values()].map(t=>({...copy(t),status:this.#state.completedTasks.includes(t.id)?'complete':this.meets(t.requires)?'active':'locked'}));}
        #publish(){window.dispatchEvent(new CustomEvent('moon:progress-changed',{detail:this.state}));}
        #present(){window.dispatchEvent(new CustomEvent('moon:story-presentation',{detail:this.current}));}
        ...
        #drain(){
            ...
            if(this.#pending.queue.length){this.#present();return;}
            const {event,draft}=this.#pending;
            this.#effects(draft,event.effects);
            if(event.once!==false)draft.oneShotEvents.push(event.id);
            this.#state=storage.migrateSave(draft);this.#pending=null;this.#publish();this.#present();
        }
        advance(){if(this.current?.type!=='line')return false;this.#pending.queue.shift();this.#drain();return true;}
        skipSegment(){
        ...
        saveCheckpoint(slot='auto'){
            ...
            return this.busy?storage.saveGame(enrichSaveState(this.state),slot):this.save(slot);
        }
        choose(id){
            const item=this.current;
            if(item?.type!=='choice')return false;
            const option=item.options.find(o=>o.id===id);if(!option)return false;
            this.#pending.draft.choices[item.id]=id;this.#effects(this.#pending.draft,option.effects);
            this.#pending.queue.shift();this.#drain();return true;
        }
        tick(seconds){
            ...
            if(this.current?.type==='wait'){
                ...
                if(this.#pending.queue[0].seconds<=0){this.#pending.queue.shift();this.#drain();}
            }
        }
        completeTask(id){
            const task=this.#tasks.get(id);
            if(!task||this.#state.completedTasks.includes(id))return false;
            return this.run({id:`task-${id}`,requires:task.requires,queue:task.queue,effects:{...task.effects,completedTasks:[...(task.effects?.completedTasks||[]),id]}});
        }
        setLocation(map,x,y){
            ...
            this.#state.currentMap=map;this.#state.playerPosition={x,y};return true;
        }
        save(slot='auto',checkpointId=this.#state.checkpointId){

            if(this.busy)return false;
            const candidate=enrichSaveState({...this.state,checkpointId});
            const saved=storage.saveGame(candidate,slot);
            if(saved)this.#state=runtimeState(saved);return saved;
        }
    }
